...
| IDN 接口       | 5 轮 ×（8 workers × 10） | 仅作诊断，不定档 | 本机 37.5～45.3 ms；最终 Linux 157.0 ms | 轮次 p95 中位数 ≤ 180 ms，错误率 0 |
...
| Linux CI，最终场景隔离口径     | 轮次 178.6、164.8、157.0、134.4、145.4；门禁值 157.0 | IDN、公开页、域名及三页 Lighthouse 均通过  |
...
选择 5 个独立轮次而不是只增加单个大批次，是为了保留每轮 8 workers 的实际并发形态，同时让一次或两次共享 runner 短时抢占不能单独主导最终值；奇数轮中位数仍会在至少 3 轮持续变慢时抬升。第二次 Linux 新口径运行中 IDN 中位数 `200.5 ms` 通过当时 215 ms 门槛，但并行执行的公开页达到 `1477.5 ms > 990 ms`；这暴露三个异质场景原先由一个 `Promise.all` 同时启动，单场景值会受其他场景争抢影响。修复将三个场景顺序隔离，单测断言最大同时活跃场景数为 1；最终本机公开页连续 5 次为 250.9、254.3、250.8、240.4、259.9 ms，标准差 6.4 ms、变异系数 2.5%，没有放宽 990 ms 门槛。最终 Linux 隔离运行中公开页/域名/IDN p95 为 `763.3/3969.7/157.0 ms`，IDN 最大单轮 p95 为 178.6 ms，三页 Lighthouse 也全部通过。报告继续保留 `roundP95Ms` 和全体最大值，任一请求失败也计入各场景全部请求的错误率，因此稳定化没有掩盖功能错误。

稳定后口径的跨环境最差有效门禁值为最终 Linux `157.0 ms`。场景隔离前的 `191.2/200.5 ms` 保留为诊断历史，不用受其他场景争抢污染的数值给最终口径定档。按既有“最差实测 + 8%～15%”方法取 `180 ms`，增加 `23.0 ms`（约 `14.6%`）有限余量。公开工具页 `990 ms`、域名接口 `4300 ms`、LCP `3500 ms`、Performance `0.78`、TBT `150 ms` 均未修改。
...
结论：当前建议继续把 `make performance` 保留为每个 PR 的阻塞门禁，不在本切片调整 CI 编排。理由是 IDN 同机变异系数已从 `35.8%` 降到 `6.1%`，最终 Linux 隔离口径也在一次自然 CI 运行中通过；顺序隔离后公开页同机变异系数为 2.5%。其余接口与三页 Lighthouse 仍是确定性 fixture/本地页面，对真实性能回归有直接拦截价值。最终顺序隔离口径仍需继续积累 Linux 样本；若仍反复出现无代码变化的假失败，或 Linux 连续样本的方差持续大于当前有限余量，再由项目负责人决定移到定时/手动执行，同时保留数值门槛；不得通过失败后反复重跑来维持 per-PR 绿色。
