...
| IDN 接口       | 5 轮 ×（8 workers × 10） | 仅作诊断，不定档 | 本机 37.5～45.3 ms；Linux 191.2～200.5 ms | 轮次 p95 中位数 ≤ 225 ms，错误率 0 |
...
IDN 最终口径的数值失败路径按要求完成变异：只把 `apps/web/performance/baseline.json` 的 IDN `maximumP95Ms` 从校准值临时改为 `0`，顺序隔离的三个接口场景与首页、域名查询页、IDN 页三页 Lighthouse 均完成，随后明确失败 `idn p95 38.1ms > 0ms`，退出码为 `1`。恢复 225 ms 后完整门禁以公开页/域名/IDN p95 `259.1/3995.6/40.9 ms` 退出 0；没有修改其他接口或 Lighthouse 门槛。
...
结论：当前建议继续把 `make performance` 保留为每个 PR 的阻塞门禁，不在本切片调整 CI 编排。理由是 IDN 同机变异系数已从 `35.8%` 降到 `6.1%`，两个 Linux 新口径 IDN 值均落在按规则设置的有限余量内；顺序隔离后公开页同机变异系数为 2.5%。其余接口与三页 Lighthouse 仍是确定性 fixture/本地页面，对真实性能回归有直接拦截价值。最终顺序隔离口径仍需继续积累 Linux 样本；若仍反复出现无代码变化的假失败，或 Linux 连续样本的方差持续大于当前有限余量，再由项目负责人决定移到定时/手动执行，同时保留数值门槛；不得通过失败后反复重跑来维持 per-PR 绿色。
