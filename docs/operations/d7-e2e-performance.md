...
正式测量环境覆盖 macOS arm64 本机与 GitHub Actions Ubuntu runner，统一使用 Node.js 24.18.0、Lighthouse/chrome-launcher 13.4.1/1.2.1、Next.js 生产构建和本地 fixture；macOS 使用系统 Chrome 151.0.7922.76，Linux CI 使用 Playwright Chromium 145.0.7632.6。公开工具页和域名接口仍分别按单批全部请求计算 p95；IDN 在丢弃 3 个预热请求后执行 5 个独立轮次，每轮保持 8 workers × 10 请求，门禁指标取 5 个轮次 p95 的中位数，错误率仍聚合全部 400 请求。Lighthouse 使用 desktop simulated throttling，每页 3 次并取中值。本机连续执行三轮初始校准/门禁：第一轮公开工具页因 p95 357.7 ms 超过初始 300 ms 门槛失败，第二轮 IDN 因旧口径单次 p95 160.4 ms 超过初始 150 ms 门槛失败，第三轮通过。首轮 Linux CI 又如实失败于公开页 810.9 ms、IDN 旧口径 274.9 ms 和三页 TBT 89.5～104 ms；第三轮 Linux 实测 IDN TBT 中位数进一步达到 131.5 ms，第四轮公开页与 IDN 旧口径 p95 分别达到 898.9 ms、317.9 ms。域名接口、错误率、LCP 与四类分数均通过。跨环境最终门槛按各项最差有效实测增加约 8%～15% 抖动空间，不按本机最快值冒充 CI 可重复基线。
...
| IDN 接口       | 5 轮 ×（8 workers × 10） | 仅作诊断，不定档 | 本机 48.3～59.3 ms；Linux 191.2 ms | 轮次 p95 中位数 ≤ 215 ms，错误率 0 |
...
| 本机同环境复测，新口径         | 56.0、52.9、48.3、48.3、59.3                         | 每个数均为一次门禁的 5 轮 p95 中位数       |
| Linux CI，新口径               | 轮次 302.6、248.0、191.2、157.6、148.3；门禁值 191.2 | 400 请求；完整 `make check` 与性能步骤通过 |
...
| 新：5 轮 p95 的中位数 |  53.0 ms |   18.6 ms² |  4.3 ms |     8.1% |  11.0 ms |

选择 5 个独立轮次而不是只增加单个大批次，是为了保留每轮 8 workers 的实际并发形态，同时让一次或两次共享 runner 短时抢占不能单独主导最终值；奇数轮中位数仍会在至少 3 轮持续变慢时抬升。报告保留 `roundP95Ms` 和全体最大值便于诊断，任一请求失败也继续计入 400 请求的聚合错误率，因此稳定化没有掩盖功能错误。

新口径跨环境最差有效门禁值为 Linux `191.2 ms`。按既有“最差实测 + 8%～15%”方法取 `215 ms`，增加 `23.8 ms`（约 `12.4%`）有限余量。公开工具页 `990 ms`、域名接口 `4300 ms`、LCP `3500 ms`、Performance `0.78`、TBT `150 ms` 均未修改。
...
IDN 新口径的数值失败路径按要求完成变异：只把 `apps/web/performance/baseline.json` 的 IDN `maximumP95Ms` 从校准值临时改为 `0`，完整流程仍测量了首页、域名查询页和 IDN 页三页 Lighthouse，随后明确失败 `idn p95 55.1ms > 0ms`，退出码为 `1`。恢复阈值后本机新口径连续门禁和 Linux CI 均通过；没有修改其他接口或 Lighthouse 门槛。
...
结论：当前建议继续把 `make performance` 保留为每个 PR 的阻塞门禁，不在本切片调整 CI 编排。理由是 IDN 同机变异系数已从 `35.8%` 降到 `8.1%`，首次 Linux 新口径也在按规则留出的有限余量内通过；其余接口与三页 Lighthouse 仍是确定性 fixture/本地页面，对真实性能回归有直接拦截价值。当前只有一次 Linux 新口径运行，证据尚不足以证明共享 runner 的跨运行方差已完全收敛；应继续如实积累后续有效 CI 样本。若稳定口径后仍反复出现无代码变化的假失败，或 Linux 连续样本的方差持续大于当前有限余量，再由项目负责人决定移到定时/手动执行，同时保留数值门槛；不得通过失败后反复重跑来维持 per-PR 绿色。
