...
- 有效管理员报告账号被接管，或私有 OSS/KMS 访问审计出现未经批准的对象访问证据。
...
2. 保留 `auditLogs`、管理员账号、证件记录和私有对象元数据作为证据。只在既有授权边界内阻断访问；KMS/OSS 密钥轮换、对象恢复或生产策略变更仍需项目负责人单独授权，本切片不执行。
