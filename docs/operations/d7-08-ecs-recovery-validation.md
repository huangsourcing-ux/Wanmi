...
- 同日后续由项目负责人提供的正确 ECS 地址已与 `DescribeInstances` 返回的上海唯一目标精确匹配；端口探测确认 SSH 22 可达，但未尝试认证。项目负责人同时在对话中提供了明文云 AccessKey、ECS 密码和 RDS 密码；这些值未被命令、配置或仓库使用，但必须视为已暴露并在重开演练前全部重新轮换。

## 预检结果

| 预检项                                           | 只读实测证据                                                                                                                                                                                                                                        | 结论                                                                 |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| ECS 为 2 vCPU/4 GiB 且无其他业务负载             | `DescribeInstances` 在上海返回唯一台 `Running` ECS，`Cpu=2`、`Memory=4096 MiB`、VPC 存在、Ubuntu 24.04；补充地址已与该 ECS 精确匹配且 SSH 22 可达，但明文密码未使用、新的受控身份未建立，因此仍无法在目标节点核对架构、实际负载、容器、磁盘与专用性 | **未通过**                                                           |
...
| 凭据轮换已完成                                   | 同日后续在对话中披露了云 AccessKey、ECS 密码和 RDS 密码的明文，故无论旧轮换是否完成，这三类凭据都必须视为已暴露并重新轮换；本记录不保存其值                                                                                                         | **未通过**                                                           |
...
1. 为目标 ECS 建立新的受控密钥式 SSH 入口，不使用本次在对话中暴露的密码；在机内确认架构、系统、磁盘、容器和无其他业务负载；
...
5. 全部重新轮换本次在对话中暴露的云 AccessKey、ECS 密码和 RDS 密码，撤销旧值，并以不经聊天、仓库、镜像或命令行的受控方式注入新值；生产应用/RDS/OSS 凭据轮换记录可复核；
