...
当前项目负责人批准并冻结的 P1 文档基线为 `P1-BASELINE-2026-08-14.2`，对应批准标签 `p1-docs-approved-2026-08-14-2`。本基线在上一基线 `P1-BASELINE-2026-08-14.1`（标签 `p1-docs-approved-2026-08-14-1`）之上修正生产发布与存储边界：只能发布已合并到 `main` 的提交；当前 linux/amd64 镜像经 SSH 直传目标 ECS、载入节点 loopback registry 并按 release manifest digest 校验；不得用对象存储中转镜像或源码，不得向实名证件存储写入任何非证件对象，也不得自行选择或创建部署设施替代缺失前置。上一基线新增的 D9 用户系统及 13 项范围决定，以及更早基线关于版本化应用主密钥、订单状态、退款、实名归属、12～16 周工期口径和生产上线门槛的决定全部继续有效。
