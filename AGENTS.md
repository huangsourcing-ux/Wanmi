...
当前项目负责人批准并冻结的 P1 文档基线为 `P1-BASELINE-2026-08-14.1`，对应批准标签 `p1-docs-approved-2026-08-14-1`。本基线在上一基线 `P1-BASELINE-2026-08-10.1`（标签 `p1-docs-approved-2026-08-10-1`）之上新增 D9 用户系统，并纳入项目负责人已确认的全部 13 项范围决定：D9-A 账户与身份阻塞首次上线与 D8 整体验收，D9-B 钱包与对账、D9-C 域名用户中心、D9-D 域名控制台增强、D9-E 增长与会员不阻塞首次上线或 D8；精确范围、排除项和退出条件以 `Wanmi.AI-P1开发计划.md` 第 16 节为准。上一基线关于移除不可用阿里云 KMS、改用版本化应用主密钥，以及 `pending_payment → manual_review`、其他订单状态与迁移、退款规则、实名归属、12～16 周工期口径和生产上线门槛的决定继续有效。
...
- 高风险交互操作使用操作密码产生的短时 `stepUpGrant`；无人值守自动续费只依赖事先创建的 `renewalMandate`。
