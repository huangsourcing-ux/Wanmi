...
当前批准并冻结的 P1 文档基线为 `P1-BASELINE-2026-08-03`，对应 Git 标签 `p1-docs-approved-2026-08-03`。
...
5. `Wanmi.AI-现有阿里云资源.md`：资源事实和上线门槛；
6. `Wanmi.AI-产品规划与商业评估.md`；
7. `Wanmi.AI-App技术规划.md`。
...
7. 汇报变更、验证结果、剩余风险和下一步。

保留用户已有修改，不覆盖、重置或删除不属于当前任务的内容。未经明确授权不得提交、推送、部署、采购云资源、申请备案、发送真实交易或调用西部数码写接口。
...
- Node.js 24 LTS；
- Next.js 16.2.6、React、TypeScript strict、App Router；
...
业务开发前以 3～5 天作为 D0 架构决策时间盒完成验证。任一退出条件未满足时必须延长 D0、记录证据并修正假设，不得为赶时间跳过：

- Next.js 16.2.6 + Payload 3.86.0 + PostgreSQL；
- 草稿、版本、定时发布、角色权限；
