...
当前项目负责人批准并冻结的 P1 文档基线为 `P1-BASELINE-2026-08-18.1`，对应批准标签 `p1-docs-approved-2026-08-18-1`。本基线在上一基线 `P1-BASELINE-2026-08-17.2`（标签 `p1-docs-approved-2026-08-17-2`）之上，只把 A4“获取/修改域名管理密码”的保护等级对齐为既有实现：purpose-bound step-up、至少一个 active 绑定渠道存在性校验，以及操作完成后向全部 active 绑定渠道告知并逐 provider 记录 outcome；不实施执行前的渠道确认。项目负责人于 2026-08-18 明确接受其安全后果：会话被盗时用户只能事后获知，不能通过绑定渠道在执行前阻断。上一基线的 DNS 高风险分级、并发互斥、D9 既有能力复用、按调用点与耦合关系计数、验证分层、`make check` 完整门禁、精确提交绿色 CI、不可变镜像 digest 发布依据，以及其之前的 P1 范围、架构、高风险冷静期、`renewalMandate` 独立授权、订单/退款/实名、工期与生产上线门槛全部继续有效。
...
| 余额支付与退款分派        | `createBalancePayment`、`captureBalancePaymentForFulfillment`、`requestAutomaticRegistrationFailureRefund`                                                                                                                                                                     | `apps/web/src/services/commerce/balance-payments.ts`；`apps/web/src/services/commerce/refunds.ts`                                                                  |
| 钱包资金规则              | `loadWalletFundsPolicy`、`updateWalletFundsPolicy`、`assertWalletCurrency`、`assertSingleTopUpLimit`、`assertAccountBalanceLimit`、`assertSingleSpendLimit`                                                                                                                    | `apps/web/src/services/wallet/policy.ts`                                                                                                                           |
| 充值退款与争议追回        | `requestWalletTopUpOriginalRefund`、`refundDuplicateWalletTopUp`、`refundOrderWhenServiceNotProvided`、`requestAccountClosureBalanceRefunds`、`recoverWalletTopUpPaymentReversal`                                                                                              | `apps/web/src/services/commerce/refunds.ts`；`apps/web/src/services/wallet/fund-scenarios.ts`                                                                      |
| 钱包账单导出              | `exportWalletStatement`；固定使用 `Asia/Shanghai` 和 `ledger_entries_start_inclusive_end_exclusive`                                                                                                                                                                            | `apps/web/src/services/wallet/statements.ts`                                                                                                                       |
| 余额第四 ledger 对账      | `reconcileWalletLedger`；只读推导 `walletEntries` 事实、复用既有 `reconciliationKey`/`manualReviews`/审计，差异不得自动纠正                                                                                                                                                    | `apps/web/src/services/commerce/reconciliation.ts`                                                                                                                 |
| 米币账本与工具额度        | `earnPendingOrderReward`、`confirmPendingOrderReward`、`reversePendingOrderReward`、`redeemPointsForToolQuota`、`consumeToolQuota`、`runPointsExpiration`；独立于钱包表、函数与幂等键空间                                                                                      | `apps/web/src/services/points/ledger.ts`                                                                                                                           |
| 永久 VIP 等级             | `publishVipTierRuleVersion`、`recordVipSpendForSucceededOrder`、`recordVipSpendReversalForRefundedOrder`、`promoteCustomerVipTier`、`applyApprovedVipTierCorrection`、`readCustomerVipStatus`、`recordVipTierCorrectionAppeal`；等级只从追加式事件高水位读取，累计消费独立推导 | `apps/web/src/services/vip/tiers.ts`                                                                                                                               |
| 后台高风险操作审批        | `createAdminApprovalRequest`、`decideAdminApprovalRequest`、`executeAdminApprovalRequest`；领域执行从 `executeSupportedAdminOperation` 进入，未接入安全领域 executor 的类型必须 fail-closed                                                                                    | `apps/web/src/services/admin/approvals.ts`；`apps/web/src/services/admin/operation-executors.ts`                                                                   |
| 交易通知 outbox           | `enqueueTransactionalSecurityNotification`、`runNotificationDeliveries`、`updateNotificationPreference`；高风险安全事件复用既有短信/微信 provider，外部失败只改变 delivery/receipt                                                                                             | `apps/web/src/services/notifications/outbox.ts`                                                                                                                    |
| 邀请绑定与版本化规则      | `generateInvitationCode`、`bindInvitationAtRegistration`、`bindCustomerInvitation`、`disableCustomerInvitationCode`、`createInvitationRewardRuleVersion`；绑定只使用服务端窗口与一次性唯一关系，旧 12 位码只兼容不再生成                                                       | `apps/web/src/services/invitations/binding.ts`；`apps/web/src/services/invitations/rules.ts`                                                                       |
| 邀请奖励与反作弊          | `processInvitationRewardForOrderTransition`、`detectInvitationAbuseSignals`、`recheckInvitationRewardClaim`、`scanReleasedInvitationRewardsForAbuse`；米币只经 E-2 ledger，信号只挂起/告警/人工复核，不自动扣回或变更账户状态                                                  | `apps/web/src/services/invitations/rewards.ts`；`apps/web/src/services/points/ledger.ts`                                                                           |
| 反滥用速率监控            | `loadOperationsMonitoringThresholds`、`collectAbuseMonitoringSnapshot`、`evaluateOperationsMonitoring`、`runOperationsMonitoring`；沿用 D7-02 `siteSettings` 阈值/CAS，只输出聚合计数与阈值命中，不落地新敏感标识                                                              | `apps/web/src/services/operations/monitoring-thresholds.ts`；`apps/web/src/services/operations/monitoring.ts`                                                      |
...
| 自动续费授权与执行        | `previewCustomerRenewalMandateChange`、`changeCustomerRenewalMandate`、`runAutomaticRenewalForAsset`、`runAutomaticRenewals`、`revalidateAutomaticRenewalOrder`；余额只复用钱包 hold，履约继续进入 D6 `commerceFulfillment`                                                    | `apps/web/src/services/domains/renewal-mandates.ts`；`apps/web/src/services/domains/automatic-renewals.ts`；`apps/web/src/services/commerce/fulfillment.ts`        |
