...
当前项目负责人批准并冻结的 P1 文档基线为 `P1-BASELINE-2026-08-15.1`，对应批准标签 `p1-docs-approved-2026-08-15-1`。本基线在上一基线 `P1-BASELINE-2026-08-14.2`（标签 `p1-docs-approved-2026-08-14-2`）之上，将 D9-A 交互式高风险操作的 `stepUpGrant` 取得方式从独立操作密码改为短信验证码校验，不引入操作密码设置或重置流程；风险分级、身份找回/换绑后的高风险冷静期，以及无人值守自动续费只能依据 `renewalMandate` 且不得与 step-up 混用的边界全部不变。上一基线关于已合并 `main` 的 SSH 镜像直传发布、对象存储与实名证件存储边界，以及更早基线关于 D9 用户系统、版本化应用主密钥、订单状态、退款、实名归属、12～16 周工期口径和生产上线门槛的决定全部继续有效。
