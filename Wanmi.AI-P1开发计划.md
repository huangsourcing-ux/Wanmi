...
D7-12 剩余契约与主密钥注入验证（2026-08-12～13 EDT / 2026-08-13 UTC）：本地契约环境与目标 ECS 当前运行容器的脱敏预检均确认 `ALLOW_REAL_WECHATPAY_PAYMENTS=false`、`ALLOW_REAL_WECHATPAY_REFUNDS=false`；总闸、微信 provider 闸和短信发送闸也均为 `false`。负责人补充的微信材料在本地通过商户号/证书主体、证书序列号、RSA 私钥/证书公钥、证书有效期和 32 字节 API v3 key 一致性校验；6 项商户侧配置写入忽略版本控制的本地配置，错误复用商户证书序列号的“平台序列号”未写入，模式保持 fixture。微信官方 CertificateDownloader 对证书列表接口的真实只读请求返回 HTTP `403` / `NOT_ENOUGH`，语义为平台证书已过期失效，耗时 3,497 ms；未生成证书且响应无法在缺少微信支付公钥时完成平台验签，所以没有把该前置响应冒充不存在订单查单，也没有修改查询 schema、错误映射或 fixture 断言。当前尚缺商户平台下载的微信支付公钥 ID/文件，目标查单未发起。短信发送闸关闭，且 ECS 缺少测试号码、凭据、Region、签名和两类模板，所以只完成缺项/门禁预检；live 配置加载和 provider 构造不通过，没有绕过 OTP 四维限频，发送数为 0。

目标 ECS 已运行，无需重建，重建耗时单独记 0 秒且不改 D7-11 RTO。在当前 Web 容器中以真实注入环境执行一次性脱敏探针：`getEnv` 对版本名、重复版本、标准 Base64、32 字节和 active membership 的完整校验通过；active version 为 `prod-20260811-v2`；新 `WANMI-RN1` 证件信封对象使用该 active version，解密回原文并精确清理，探针耗时 7.727 秒。本次未打开任何真实能力闸，未发起下单、支付、退款、短信或 Bucket 请求。WestDigital、私有 OSS 和主密钥注入已有真实证据，但微信与短信未完成，故 11.1 第 2 项继续保持未勾选；第 13 项和生产硬门槛未修改。完整脱敏证据及已披露云/provider 凭据轮换提醒见 `docs/operations/d7-05-provider-read-contracts.md`。
...
| 2026-08-12 | D7-12 剩余契约与主密钥注入          | 脱敏预检微信/短信门禁与配置；校验并本地注入 6 项微信商户侧配置；目标 ECS 真实 `getEnv`/key ring 校验与 `WANMI-RN1` 证件信封加解密往返                                                         | 资金写闸始终为 `false`；微信证书前置返回 `403/NOT_ENOUGH`（3,497 ms）；主密钥 active version `prod-20260811-v2`，信封往返/清理通过（7.727 秒）；`make check` 639/639 单元、105/105 集成及完整构建/安全门禁通过                                              | 微信尚缺公钥 ID/文件与验签查单，短信缺凭据/模板/测试号码且发送 0；11.1 第 2 项保持未勾选，第 13 项未改                                                                |
