...
D7-03 IDN 性能门禁稳定性复审（2026-08-12）：PR #63 在生产代码未变化时得到 Linux CI IDN 旧口径 `371.2 ms > 350 ms`，与此前有效的 274.9、306.2、317.9 ms 一并证明单次 8 workers × 10 请求 p95 已超出原有限余量。修复只改变 IDN 统计：丢弃原有 3 个预热请求后执行 5 个独立的 80 请求轮次，门禁取轮次 p95 中位数，保留逐轮 p95/全体最大值并把错误率聚合到全部 400 请求；公开页、域名和 Lighthouse 口径均不变。同一生产构建连续 5 次旧口径为 141.4、83.7、187.4、71.7、104.2 ms，新口径为 56.0、52.9、48.3、48.3、59.3 ms，总体标准差从 42.1 降到 4.3 ms，变异系数从 35.8% 降到 8.1%。PR #65 首次 Linux 新口径 5 轮为 302.6、248.0、191.2、157.6、148.3 ms，中位数 191.2 ms；按最差有效值增加 12.4% 有限余量，将 IDN 门槛重定为 215 ms。公开工具页 990、域名 4300、LCP 3500、Performance 0.78、TBT 150 均未修改。把 IDN 门槛临时变异为 0 后，三页 Lighthouse 均完成，实际失败 `idn p95 55.1ms > 0ms` 且退出码 1；恢复后完整 `make performance` 以三组 p95 450.8/3974.7/56.3 ms 通过。最终全 fixture/mock、全部真实能力闸 `false` 的 `make check` 退出码 0，通过 84 文件 620/620 单元、28 文件 105/105 集成、全部 migration 往返、生成物/schema、运维/发布/重建契约、生产构建、linux/amd64 镜像及依赖/秘密/漏洞门禁。当前建议继续保留 per-PR 阻塞门禁并积累 Linux 样本；若新口径仍反复发生无代码变化的假失败，再由项目负责人决定改为定时/手动执行，不以反复重跑规避失败。完整分布、方差与变异证据见 `docs/operations/d7-e2e-performance.md`；未修改 PR #63 的测试、D7-09 演练结论或勾选状态。
