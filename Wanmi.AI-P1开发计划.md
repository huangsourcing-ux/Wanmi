...
D8 验收（一）记录（2026-08-21）：M01～M15 完全满足，M16 因 Name Server 写入口按方案 B 禁用，
且提醒偏好、续费授权/执行与 D9 域名控制台尚无用户前端闭环，判定为部分满足，第一条保持未勾选。
全量静态扫描另发现生产零调用领域能力、完全零引用导出、平行反馈 Collection、直接冗余依赖和
未接入当前流程的一次性脚本等 a 类项，第八条保持未勾选。逐项证据、精确差距和 a/b/c 分类见
`docs/operations/d8-module-acceptance.md`；本次只审计与记录，不删除或修改任何业务实现。
...
- [x] D9-C 域名用户中心（不阻塞首次上线与 D8）
- [x] D9-D 域名控制台增强（不阻塞首次上线与 D8）
...
- [x] 服务号已完成服务器配置（消息回调 URL、Token、EncodingAESKey），回调公网 HTTPS 可达并
      通过微信验证 (负责人 2026-08-21 确认完成)；
- [x] 服务号具备「生成带参数二维码」「网页授权」「客服/模板消息」接口权限 (负责人 2026-08-21 确认完成)；
- [x] 网页授权域名已配置 (负责人 2026-08-21 确认完成)；
- [x] **阿里云验证码 2.0** 已开通并取得场景配置 (负责人 2026-08-21 确认完成)；
- [x] 凭据以受控方式注入运行环境，不进入仓库 (负责人 2026-08-21 确认完成)。
...
- [x] `pointsLedger` 追加式；**生命周期** `pending → available → held → consumed`，另有
  ...
  - **实现与行为证据**：`apps/web/src/collections/points.ts:148-198` 定义独立追加式
    ...
    `apps/web/tests/integration/d9e2-points-ledger.integration.test.ts:402` 用例“keeps a
    succeeded-order reward pending and unavailable until confirmation”与 `:609` 用例“reverses a
...
- [x] **批次分配**：不同批次过期时间不同，消费按**最早过期优先**，记录
  ...
  - **实现与确定性证据**：`apps/web/src/services/points/ledger.ts:805` 明确使用
    `expires_at ASC, id ASC`，`:887` 对持久 allocation 使用 `expires_at ASC, batch_id ASC`，`:998-1030`
    在同事务追加 allocation，`:1470` 对过期候选使用 `expires_at ASC, id ASC`。测试
    `apps/web/tests/integration/d9e2-points-ledger.integration.test.ts:678` 用例“allocates
    equal-expiry spendable batches by ascending id and replays identically”、`:735` 用例“returns persisted
    equal-expiry allocations by ascending batch id”及 `:1880` 用例“expires equal-time batches by ascending
    id and honors the exact batch limit”分别保护三处 id 兜底；每条均构造同 expiry、不同 id 且相反物理顺序的
    批次或 allocation，只删除对应兜底时严格为对应 `1 failed | 2 passed`。`:769` 用例“recomputes
    remaining expirable points from cross-batch allocations”构造跨批次消费并从 allocation 重算剩余过期额度。
- [x] 赚取幂等（幂等键 + 唯一索引）；消耗原子不透支；过期由 job 追加冲销条目，不改历史条目；
  - **幂等、并发与过期证据**：migration
    ...
    `apps/web/tests/integration/d9e2-points-ledger.integration.test.ts:446`、`:1765`、`:1804`、`:2000`
...
- [x] **兑换目标为工具额度类权益**（高级 Whois 查询次数、批量查询额度、AI 域名分析报告次数）；
  ...
  - **目标与金额隔离证据**：`apps/web/src/collections/points.ts:7` 只允许 `advanced_whois`、
    ...
    分别追加额度发放和使用记录。测试 `apps/web/tests/integration/d9e2-points-ledger.integration.test.ts:2044`
    用例“grants only approved tool quotas and atomically prevents quota over-consumption”拒绝其他目标并验证
    额度原子扣减，`:2430` 用例“does not change an order payable amount when points are redeemed”逐项确认
...
- [x] 米币与余额严格隔离，不可互换。
  - **数据层与入口隔离证据**：`apps/web/src/collections/points.ts:58-275` 与 migration
    ...
    `apps/web/tests/integration/d9e2-points-ledger.integration.test.ts:2379` 用例“has no points-wallet
  ...
  - **全判定点、事实来源与风险分级证据**：
    ...
    数据来源替换变异均由指定行为断言杀死；同文档第 2.1 节记录三处删除兜底的原始报错与各自
    `EXCLUSIVE_FAILURE 1/3`，第 2.2 节逐一审计其它排序和迭代顺序；第 3 节逐字段记录 fixture 去相关，
    第 4 节逐行对照 A4 九档并确认未新增或降低 step-up 风险档。恢复源码后 D9-E-2 聚焦 41/41，通过
    最终状态完整 `make check` 的 845/845 单元、745/745 主集成、37/37 独立钱包账本集成与 11/11
    独立钱包对账集成（集成合计 793/793）。
