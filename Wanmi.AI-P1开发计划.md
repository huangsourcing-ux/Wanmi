...
`P1-BASELINE-2026-08-17.1` 冻结 P1 产品范围、固定架构、订单状态与合法迁移、退款规则、实名归属、12～16 周工期口径、生产上线门槛、发布/存储边界、风险匹配的验证治理，以及 `AGENTS.md` 中的并发互斥与 D9 既有能力复用约束。相对上一基线 `P1-BASELINE-2026-08-16.1`（批准标签 `p1-docs-approved-2026-08-16-1`），本次只新增同事务条件更新、`RETURNING` 授权判定、整数分/BigInt、行锁真源、追加式记录、上游 unknown 只查询、D9 复用索引，以及按调用点和耦合关系计数的判定点变异口径；上一基线的验证分层、D9-A 短信 step-up、风险分级、高风险冷静期、`renewalMandate` 独立授权，以及更早基线的全部决定继续有效。开发过程中可以更新本计划的任务勾选、验证证据、阻塞、ADR 和 Runbook；不得借进度更新改变冻结基线。
