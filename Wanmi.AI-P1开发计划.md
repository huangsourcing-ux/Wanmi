...
`P1-BASELINE-2026-08-16.1` 冻结 P1 产品范围、固定架构、订单状态与合法迁移、退款规则、实名归属、12～16 周工期口径、生产上线门槛、发布/存储边界及风险匹配的验证治理。相对上一基线 `P1-BASELINE-2026-08-15.1`（批准标签 `p1-docs-approved-2026-08-15-1`），本次只新增验证分层、CI 失败关闭、定向变异证据和 PR 报告要求；上一基线的 D9-A 短信 step-up、风险分级、高风险冷静期、`renewalMandate` 独立授权，以及更早基线的全部决定继续有效。开发过程中可以更新本计划的任务勾选、验证证据、阻塞、ADR 和 Runbook；不得借进度更新改变冻结基线。
...
| --- | --- |
| 添加普通子域解析 | 当前会话 + 审计 |
| 修改根域 A/CNAME、MX、NS | step-up + 二次确认 |
| 批量删除解析 | step-up + 变更预览 |
| 关闭域名锁 | step-up + 通知 |
| 修改实名信息 | step-up + 二次确认 |
...
- [ ] 新增 `accountClosureRequests`：申请时间、原因、当前阻塞项、冷静期、撤销时间、最终执行
      时间、数据保留与匿名化结果；
- [ ] **持有域名时不得直接完成注销**，只能进入关闭申请或受限状态，须先完成域名转移、过户或
      其他资产处置；
- [ ] 其他前置检查：未完成订单、待执行自动续费、退款或对账差异、发票处理中、安全冻结或争议、
      正余额；
- [ ] 身份解除后多久可被另一账号重新绑定，须显式定义并记录。
...
- [ ] `orders.paymentChannel` 新增 `balance`；余额支付即时，无 `code_url`、不轮询、不超时关单；
      其「服务端确认」等价于**同一事务内成功的原子扣减**；
- [ ] 混合支付（余额 + 微信同单）**P1 不做**；
- [ ] 退款：余额支付退回余额；微信支付走 D5-04 原路退款；两条路径不得互串；退款金额一律以订单
      冻结金额为准。
