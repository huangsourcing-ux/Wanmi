...
2026-08-17 冻结项变更只在 `AGENTS.md` 固化并发互斥规则、D9 既有能力复用索引与按调用点/耦合关系计数的判定点口径；既有状态机、Payload/PostgreSQL 架构、验证分层和生产上线门槛不变。

2026-08-17 第二次冻结项变更只扩展 A4 DNS 高风险分级：根域 AAAA 与全部主机 TXT 进入既有 purpose-bound step-up + 二次确认链路，不新增 purpose、状态机、架构或生产权限。

2026-08-18 的 `P1-BASELINE-2026-08-18.1` / `p1-docs-approved-2026-08-18-1` 显式链回 `P1-BASELINE-2026-08-17.2` / `p1-docs-approved-2026-08-17-2`，只把 A4 域名管理密码保护措辞对齐既有实现：purpose-bound step-up、至少一个 active 绑定渠道存在性校验，操作完成后向全部 active 绑定渠道告知并逐 provider 记录 outcome；不新增执行前渠道确认。项目负责人明确接受会话被盗时只能事后获知、不能执行前阻断的安全后果；不改实现、架构或生产权限。
...
| 阿里云接口 | Alibaba Cloud TypeScript SDK 调用短信与验证码 2.0；`ali-oss` 操作实名私有对象；不依赖阿里云 KMS |
...
flowchart LR
  ...
  J --> O
  W --> H["Who-Dat"]
  J --> X["微信支付 / 微信服务号 / 西部数码 / 阿里云短信与验证码 2.0"]
```
...
- 媒体、广告主、素材、广告位和排期；
- 管理员、普通用户、登录身份、实名模板、用户会话、账户状态/限制、step-up、找回、注销和同意记录；
- 价格规则、报价、订单、支付、退款、钱包追加式账本、米币、VIP、邀请、provider 操作、域名资产、续费授权、DNS/NS 变更、人工复核、对账和审计；
...
- `aliyuncaptcha` adapter 通过 Alibaba Cloud TypeScript SDK 接入阿里云验证码 2.0，transport 可注入；只在短信发送、服务号二维码创建或异常高频刷新前校验，生产缺少真实场景配置时 fail-closed；
- `wechatofficial` adapter 负责服务号网页授权、带参数二维码、消息回调验签和客服/模板消息，transport 可注入；P1 不引入微信开放平台网站应用；
- 服务号配置必须包含回调 URL、Token、EncodingAESKey、网页授权域名及相应接口权限；凭据仅通过受控环境注入；
- 外部消息投递失败不得改变交易或资产状态，身份授权、scene、回调验签与消息投递分别记录可审计请求标识。
...
D9 新增 Collection 的精确字段、索引、状态和阶段边界以《P1 开发计划》第 16 节为准。本次只冻结规划，不创建 Collection、migration 或生成类型；后续每个实现切片必须以命名 migration 覆盖空库、历史回填和 down/up 往返。
...
D9-A 在同一 `customers.id`、opaque Session 和 Custom Strategy 上扩展，不建立平行认证或会话体系：

- 注册与登录分离，禁止静默建号；注册保存默认客户类型、年龄/企业授权声明、条款同意、来源和可选邀请码；
...
- D9-B 余额采用充值订单、追加式账本与 `posted/held/available` 三态，作为第四 ledger 对账；余额支付与微信不得混合，退款路径不得互串，余额不得自由提现、转账或变现；订单状态集合保持不变，余额支付的服务端确认等价于同一事务内成功的原子扣减。
...
- D9-B～D9-E 的账本并发、续费 mandate、DNS/域名高风险操作、米币批次、VIP 最高水位和邀请反作弊；
...
- Payload secret、Session pepper、TOTP 加密密钥、实名应用主密钥、微信 API v3 key、商户私钥、微信服务号 AppSecret/回调密钥、西部数码凭据、短信与验证码 2.0 凭据和 OSS 凭据不得进入 Git；
...
- D9-A 所需服务号服务器配置、网页授权域名、带参数二维码/网页授权/客服或模板消息权限和阿里云验证码 2.0 场景配置通过真实环境验证；
- 项目负责人完成最终上线批准。

## 14. 明确不进入 P1

- 转入转出、完整 DNS 托管、DNSSEC、注册 DNS/glue records、DDNS、常用邮箱解析和默认自动续费；
...
智能 DNS、隐私保护和 CNNIC 隐私保护是带价格/续费链路的付费增值产品，不属于 D9；建议单列后续阶段，不得借 D9-D 域名控制台实现。
...
| 版本 | 日期 | 结论 |
|---|---|---|
| v5.13 | 2026-08-18 | A4 管理密码档位对齐既有 step-up、active 绑定渠道存在性校验与事后逐 provider 告知；明确不实施执行前渠道确认及其安全后果，不改代码、架构或生产权限 |
| v5.12 | 2026-08-17 | A4 DNS 高风险档新增根域 AAAA 与全部主机 TXT，复用既有 `dns_record_change` purpose；不改变 MX、根域 A/CNAME、NS 保护及其他架构、验证和上线门槛 |
| v5.11 | 2026-08-17 | `AGENTS.md` 新增 `payload.update({ where })` 非 CAS、同事务 `UPDATE ... WHERE ... RETURNING`、整数分/BigInt、行锁真源、追加式记录和上游 unknown 只查询规则，固定 D9 既有能力复用索引，并补充判定点按调用点和耦合关系计数；不改变业务状态、验证与上线门槛 |
