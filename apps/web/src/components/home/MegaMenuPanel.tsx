...
import { AngleRightIcon, ArrowRightIcon } from '@/components/home/shared/icons'
  ...
  return (
    <div className="min-h-[327px] w-full bg-[#031242] pt-[45px]">
      <div className="flex px-[85px]">
        ...
        >
          <span className="text-[24px] font-medium leading-tight text-white">{panel.title}</span>
          {panel.lead ? (
            <a href="#" className="text-base text-white/80 transition-opacity hover:opacity-100">
              {panel.lead}
            </a>
          ) : null}
          ...
          {panel.promo ? (
            <div className="mt-4 flex flex-col gap-1 rounded-2xl border border-white/15 bg-white/5 p-4">
              <span className="text-[18px] font-medium text-white">{panel.promo.title}</span>
