...
import {
  accountClosureExecuteResponseSchema,
...
async function readBody(request: Request): Promise<unknown> {
  if (request.headers.get('content-type')?.split(';', 1)[0]?.trim() !== 'application/json') {
    throw new AppError('UNSUPPORTED_MEDIA_TYPE', '仅接受 JSON 账户关闭执行请求', 415)
  }
...
export async function POST(request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  try {
    ...
    const input = accountClosureExecuteSchema.parse(await readBody(request))
    const payload = await getPayload({ config })
    const { req, user } = await systemAdminRequest(payload, request)
    const result = await executeAccountClosure(req, {
      actorId: user.id,
      note: input.note,
      requestId,
    })
    return successResponse(accountClosureExecuteResponseSchema.parse(result), traceId)
  } catch (error) {
