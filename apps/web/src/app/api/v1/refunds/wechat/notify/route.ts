...
import { processWechatRefundNotification } from '@/services/commerce/refunds'
...
async function readBody(request: Request): Promise<string> {
  if (request.headers.get('content-type')?.split(';', 1)[0]?.trim() !== 'application/json') {
    throw new AppError('UNSUPPORTED_MEDIA_TYPE', '仅接受微信退款 JSON 通知', 415)
  }
  const declared = Number(request.headers.get('content-length') ?? 0)
  if (declared > MAX_BODY_BYTES) {
    throw new AppError('WECHATPAY_REFUND_NOTIFICATION_TOO_LARGE', '退款通知过大', 413)
  }
...
export function createWechatRefundNotificationPostHandler(dependencies: Dependencies) {
...
export const POST = createWechatRefundNotificationPostHandler({
