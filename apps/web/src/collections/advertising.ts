...
import {
  ...
  AD_TARGET_CHECK_FAILURES,
  AD_TARGET_CHECK_STATUSES,
  ADVERTISER_STATUSES,
...
export const AdCreatives: CollectionConfig = {
  ...
  fields: [
    ...
    {
      name: 'targetCheckStatus',
      type: 'select',
      access: { create: () => false, update: () => false },
      admin: {
        description: '系统定期复检目标白名单、公共网络地址和可达性；待检或异常素材不投放。',
        readOnly: true,
      },
      defaultValue: 'pending',
      index: true,
      options: [...AD_TARGET_CHECK_STATUSES],
      required: true,
    },
    {
      name: 'targetCheckFailure',
      type: 'select',
      access: { create: () => false, update: () => false },
      admin: { readOnly: true },
      defaultValue: 'none',
      options: [...AD_TARGET_CHECK_FAILURES],
      required: true,
    },
    {
      name: 'targetCheckedAt',
      type: 'date',
      access: { create: () => false, update: () => false },
      admin: { readOnly: true },
      index: true,
